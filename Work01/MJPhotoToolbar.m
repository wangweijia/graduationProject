...
- (void)setPhotos:(NSArray *)photos
{
    ...
    CGFloat btnX = self.frame.size.width - btnWidth - 20;
    _attitudeBtn.frame = CGRectMake(btnX, 0, btnWidth, btnWidth);
    _saveImageBtn.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    //timeline_icon_unlike
    [_attitudeBtn setImage:[UIImage imageWithName:@"timeline_icon_unlike"] forState:UIControlStateNormal];
    [_attitudeBtn addTarget:self action:@selector(attitudeBtn_Click:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)attitudeBtn_Click:(UIButton *)sender{
    if ([self.delegates respondsToSelector:@selector(PhotoToolAttitudeBtn_Click)]) {
        [self.delegates PhotoToolAttitudeBtn_Click];
    }
}
